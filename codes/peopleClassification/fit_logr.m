...
function [predictions, phi] = fit_logr (X, w, var_prior, X_test, initial_phi, bay_state)
    % Find the MAP estimate of the parameters phi.
    % Optimset creates an options structure to pass to some MATLAB
    % optimization functions.
    options = optimset('GradObj','on','Hessian','on');
    phi = fminunc(@(phi) fit_logr_cost(phi, X, w, var_prior, bay_state), ...
                  initial_phi, options);
